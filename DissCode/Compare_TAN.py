...
def compare_TAN(model, parameters1, parameters2, network, k_order, no_snapshots):
        
    total_dist_per_simulation = []
    
    result1 = model.forward_simulate(parameters1, 1)
    result2 = model.forward_simulate(parameters2, 1)
    
    
    result1_ss = np.array_split(result1[0], no_snapshots)
    ...
    result2_ss = np.array_split(result2[0], no_snapshots)
